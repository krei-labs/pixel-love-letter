...
const galleryPhotos = document.querySelectorAll('.gallery__img');
...
// Setup Gallery Click Navigation
let galleryInitialized = false;
function setupGallery(){
  if (galleryInitialized) return;
  galleryInitialized = true;
  photoFrame.addEventListener('click', () => {
    galleryPhotos[currentPhotoIndex].classList.remove('active');
    currentPhotoIndex = (currentPhotoIndex + 1) % galleryPhotos.length;
    galleryPhotos[currentPhotoIndex].classList.add('active');
    
    if (!reduceMotion) heartBurst(6);
  });
}
...
function sayYes(){
  ...
  // Hide the interactive Yes/No buttons
  buttons.hidden = true;
  
  // REVEAL the surprise polaroid gallery ONLY now!
...
function heartBurst(n = 22){
  const cx = catCanvas.offsetLeft + catCanvas.offsetWidth / 2;
  const cy = catCanvas.offsetTop  + catCanvas.offsetHeight / 2;
  spawnBurst(content, cx, cy, n);
}
...
function reset(){
  catState = 'idle';
  title.textContent = 'Happy 2 months, my love ♡ Still love me?';
  ...
  // ENSURE the gallery is completely hidden upon resetting/opening
  gallery.hidden = true; 
